namespace WzHack.Loader;

using Microsoft.Extensions.Logging;
using WzHack.Core.Memory;
using WzHack.Core.Features;
using WzHack.Core.Overlay;
using WzHack.Core.Config;
...
public static class Program
{
    public static async Task Main(string[] args)
    {
        ...
        log.LogInformation("waiting for cod.exe ...");
        int pid = ProcessFinder.WaitForProcess("cod", TimeSpan.FromSeconds(120));
        if (pid <= 0)
        {
        ...
        var profiles = new ProfileManager("warzone-hack");
        ...
        log.LogInformation("attaching overlay to 'Call of Duty®'");
        if (!overlay.AttachToWindow("Call of Duty®"))
